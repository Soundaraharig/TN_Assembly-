...
class StorageService {
  ...
  public dedupeInFlight<T>(key: string, fn: () => PromiseLike<T> | Promise<T>): Promise<T> {
    ...
    const promise = (async () => {
      try {
        ...
        this.inFlightReadPromises.delete(key);
      }
    })();
  ...
  private unpackEventState(events: CollegeEvent[]): {
    ...
    events.forEach(ev => {
      ...
      if (Array.isArray(sc.questions)) {
        const validQs = sc.questions.filter((q: any) => 
  ...
  public unpackAndApplyEventState(events: CollegeEvent[], targetEventId?: string): {
    ...
    if (allQs && allQs.length > 0) {
      ...
      const localQs = this.getItem<ParliamentQuestion[]>(STORAGE_KEYS.QUESTIONS, []).filter(
      ...
      allQs.filter(q => !deletedQIds.has(q.id) && (q.status as any) !== 'Deleted' && !(q as any).deleted).forEach(q => qMap.set(q.id, q));
    ...
    if (allProceedingsQs && allProceedingsQs.length > 0) {
      ...
      const validRemotePQs = allProceedingsQs.filter(
        remoteQ => !deletedQIds.has(remoteQ.id) && (remoteQ.status as any) !== 'Deleted' && !(remoteQ as any).deleted
      );
  ...
  public async syncFromSupabase(targetEventId?: string, force = false): Promise<void> {
    const sb = supabase;
    if (!sb) {
      this.isHydrated = true;
      return;
  ...
  private async executeSyncFromSupabase(targetEventId?: string, cacheKey?: string): Promise<void> {
    ...
    try {
      ...
      if (targetEventId) {
        learnersQuery = learnersQuery.eq('event_id', targetEventId).order('created_at', { ascending: false });
      } else {
        learnersQuery = learnersQuery.order('created_at', { ascending: false });
      }

      let partiesQuery = sb.from('political_parties').select(SUPABASE_COLUMNS.POLITICAL_PARTIES);
      if (targetEventId) partiesQuery = partiesQuery.eq('event_id', targetEventId);

      let commQuery = sb.from('committees').select(SUPABASE_COLUMNS.COMMITTEES);
      if (targetEventId) commQuery = commQuery.eq('event_id', targetEventId);

      let agendaQuery = sb.from('session_agenda').select(SUPABASE_COLUMNS.SESSION_AGENDA).order('time', { ascending: true });
      if (targetEventId) agendaQuery = agendaQuery.eq('event_id', targetEventId);

      let juryQuery = sb.from('jury_members').select(SUPABASE_COLUMNS.JURY_MEMBERS);
      if (targetEventId) juryQuery = juryQuery.eq('event_id', targetEventId);

      let volQuery = sb.from('volunteers').select(SUPABASE_COLUMNS.VOLUNTEERS);
      if (targetEventId) volQuery = volQuery.eq('event_id', targetEventId);

      let daysQuery = sb.from('event_days').select(SUPABASE_COLUMNS.EVENT_DAYS).order('day_number', { ascending: true });
      if (targetEventId) daysQuery = daysQuery.eq('event_id', targetEventId);

      let attQuery = sb.from('event_day_attendance').select(SUPABASE_COLUMNS.EVENT_DAY_ATTENDANCE);
      if (targetEventId) attQuery = attQuery.eq('event_id', targetEventId);
      ...
      let extendedLearnerMap = new Map<string, any>();
      ...
      if (eventsErr) {
        ...
        const unpacked = this.unpackEventState(events as unknown as CollegeEvent[]);
        const openNomMap = unpacked.openNomMap;
        let allNoms = unpacked.allNoms;
        let allElecs = unpacked.allElecs;
        ...
        let allChecklist = unpacked.allChecklist;
        let allTeam = unpacked.allTeam;
        extendedLearnerMap = unpacked.extendedLearnerMap;
        ...
        if (events.length === 0) {
          ...
          if (targetEventId) {
            localEvents.forEach(e => {
              if (e.id !== targetEventId) eventMap.set(e.id, e);
            });
          }
   ...
   */
  public async fetchEventMetadata(eventId: string): Promise<CollegeEvent | null> {
    const cachedEv = this.getEvents().find(e => e.id === eventId);
    if (cachedEv && cachedEv.social_coverage) return cachedEv;

    const sb = supabase;
    if (!sb || !eventId) return null;
    const cacheKey = `meta_${eventId}`;
    ...
    return this.dedupeInFlight(cacheKey, async () => {
      try {
        ...
        console.warn('[StorageService] fetchEventMetadata error:', err);
      }
      return null;
    });
  }
  ...
  public async fetchVolunteerPortalData(eventId: string, volunteerId: string, force = false): Promise<void> {
    ...
    return this.fetchCached<void>(cacheKey, async () => {
      await this.executeVolunteerPortalFetch(eventId, volunteerId, cacheKey);
      await this.fetchProceedingsQuestionsOnDemand(eventId);
    }, { force });
  }
  ...
  /**
   * Paginated learner fetch supporting .limit(25) with search/filter parameters.
   */
  public async fetchPaginatedLearners(
    eventId: string,
    options: { page?: number; limit?: number; search?: string; partyId?: string; committeeId?: string } = {}
  ): Promise<{ data: Learner[]; total: number }> {
    if (!supabase || !eventId) {
      const local = this.getLearners(eventId);
      return { data: local.slice(0, 25), total: local.length };
    }
    ...
    try {
      let query = supabase
        .from('learners')
        .select(SUPABASE_COLUMNS.LEARNERS, { count: 'exact' })
        .eq('event_id', eventId);
      ...
      if (error) {
        console.warn('[StorageService] fetchPaginatedLearners query error:', error);
        return { data: [], total: 0 };
      }

      if (data && data.length > 0) {
        const local = this.getLearners();
        const fetchedIds = new Set(data.map((d: any) => d.id));
        this.setItem(STORAGE_KEYS.LEARNERS, [...local.filter(l => !fetchedIds.has(l.id)), ...(data as unknown as Learner[])]);
      }

      return { data: (data || []) as unknown as Learner[], total: count || 0 };
    } catch (err) {
      console.warn('[StorageService] fetchPaginatedLearners error:', err);
      return { data: [], total: 0 };
    }
  }
        ...
        .on('broadcast', { event: 'question_deleted' }, (msg: any) => {
          if (msg?.payload?.questionId) {
            ...
            if (!deletedQIds.includes(qId)) {
              ...
              this.setItem(STORAGE_KEYS.DELETED_QUESTION_IDS, deletedQIds);
            }
            const localPQs = this.getItem<ProceedingsQuestion[]>(STORAGE_KEYS.PROCEEDINGS_QUESTIONS, []);
  ...
  public async setStudentDayAttendance(
    ...
    const baseId = prior?.id || genUuid();
    const record: DayAttendanceRecord = {
      ...(prior || {}),
      id: baseId,
      event_id: eventId,
      day_id: dayId,
    ...
    if (typeof window !== 'undefined') {
      try {
        window.dispatchEvent(new StorageEvent('storage', { key: STORAGE_KEYS.DAY_ATTENDANCE }));
      } catch { }
    }
  ...
  public getQuestions(eventId?: string): ParliamentQuestion[] {
    const deletedQIds = this.getAllDeletedQuestionIds();
    const raw = this.getItem<ParliamentQuestion[]>(STORAGE_KEYS.QUESTIONS, INITIAL_QUESTIONS);
    const all = raw.filter(q => !deletedQIds.has(q.id) && (q.status as any) !== 'Deleted' && !(q as any).deleted);
    ...
    try {
      ...
      if (error) {
        ...
        if (remoteDeletedQIds.length > 0) {
          ...
          const mergedDeleted = Array.from(new Set([...curDeleted, ...remoteDeletedQIds]));
          this.setItem(STORAGE_KEYS.DELETED_QUESTION_IDS, mergedDeleted);
        }
        const deletedQIds = this.getAllDeletedQuestionIds();
        ...
        const otherEventsPQs = allLocalPQs.filter(q => {
          ...
          return !isThisEvent && !deletedQIds.has(q.id) && (q.status as any) !== 'Deleted' && !(q as any).deleted;
        });
  ...
  public deleteProceedingsQuestion(questionId: string, fallbackEventId?: string): void {
    ...
    // Remove from local PROCEEDINGS_QUESTIONS and QUESTIONS list immediately
    const list: ProceedingsQuestion[] = this.getItem(STORAGE_KEYS.PROCEEDINGS_QUESTIONS, []);
    ...
    const qsList = this.getItem<ParliamentQuestion[]>(STORAGE_KEYS.QUESTIONS, []);
    if (qsList.some(q => q.id === questionId)) {
      this.setItem(STORAGE_KEYS.QUESTIONS, qsList.filter(q => q.id !== questionId));
    }
