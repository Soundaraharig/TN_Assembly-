  ...
  return (
    <header className="header-theme sticky top-0 z-40 px-3 sm:px-4 lg:px-8 py-2.5 shadow-sm">
      <div className="max-w-full mx-auto flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          {(role === 'coordinator' || role === 'super_admin') && onToggleMobileMenu && (
            <button
              ...
              className="lg:hidden p-2 rounded-xl border text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer transition-colors"
        ...
        {/* Center: Cloud Status Indicator & Active Event ID Badge */}
        <div className="flex items-center gap-2">
          ...
          <div
            ...
            title={
              isSupabaseEnabled
                ? 'Connected to Supabase Cloud Database (Live Writes & Sync Active)'
                : 'Running in Local Cache Mode (Supabase not configured in .env)'
            }
          >
            {isSupabaseEnabled ? (
              <>
                ...
                <span className="hidden sm:inline font-bold text-[11px]">Cloud Sync</span>
              </>
            ) : (
              <>
                <span className="w-2 h-2 rounded-full bg-amber-500 dark:bg-amber-400 shrink-0" />
                <CloudOff className="w-3.5 h-3.5 shrink-0" />
                <span className="hidden sm:inline font-bold text-[11px]">Local Cache</span>
              </>
            )}
          </div>
          ...
          {currentEvent && (
            ...
            >
              <span className="text-[9px] uppercase font-black tracking-wider px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-600 dark:text-amber-400 font-sans">
                EVENT ID
              </span>
              ...
              {currentEvent.college_name && (
                <span className="text-slate-600 dark:text-slate-400 font-sans hidden xl:inline max-w-[180px] truncate text-[11px]">
                  • {currentEvent.college_name}
                </span>
              )}
            </div>
